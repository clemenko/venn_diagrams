...
for ext in $(cat hosts.txt|awk '{print $3}'); do
 until [ $(ssh -o ConnectTimeout=1 mashuser@$ext 'exit' 2>&1 | grep 'timed out' | wc -l) = 0 ]; do echo -n "." ; done
done
echo ""
...
token=$(curl -sk "https://$server/auth/login" -X POST -d '{"username":"admin","password":"'$password'"}'|jq -r .auth_token)
...
token=$(curl -sk "https://$server/auth/login" -X POST -d '{"username":"admin","password":"'$password'"}'|jq -r .auth_token)
curl -sk -H "Authorization: Bearer $token" https://$server/api/clientbundle -o bundle.zip
